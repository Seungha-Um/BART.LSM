...
net_vec <- seq(2, 14, 2)
rep_iter <- 100
out_in1 <- out_in2 <- out_in3 <- out_rep1 <- out_rep2 <- out_rep3 <- list()

net_num_function <- function(rep_iter, correct = FALSE, incorrect=FALSE, LSM = FALSE){
  ...
  for(rr in 1:rep_iter){
    ...
    for(q in 1:length(net_vec)){
      ...
      if(correct == TRUE){
        ...
        Z_beta <- list()
        for(g in 1:num_grp){
          Z_beta[[g]] <- cbind(1, est_Z[[g]][,1:2]) %*% beta_mat[g,]
        }
        ...
        for(s in 1:sim_num){
          ...
          beta_mat_new <- matrix(NA, num_grp, 3)

          est_Z <- XX

          for(g in 1:num_grp){
            ZZ_g <- cbind(1, est_Z[[g]][,1:2])
          ...
          if(s %% 100 == 0) cat("MCMC iter :", s, "number of net",  net_vec[q] ,"\n")
        }
        ...
        est_median <- list()
        for(k in 1:num_grp){
          est_median[[k]] <- apply(ZZlist[[k]], c(1,2), median)
        }
      ...
      out_in1[[q]] <- data.frame(out1, net = net_vec[q])
      out_in2[[q]] <- data.frame(out2, net = net_vec[q])
      out_in3[[q]] <- data.frame(out3, net = net_vec[q])
      #saveRDS(out, paste0("semiBART_correct_net", net_vec[q], "iter", rr, ".rds"))
    }# for loop for a vector of interest
    out_rep1[[rr]] <- data.frame(list.rbind(out_in1), iter = rr)
    out_rep2[[rr]] <- data.frame(list.rbind(out_in2), iter = rr)
    out_rep3[[rr]] <- data.frame(list.rbind(out_in3), iter = rr)
  }# for loop for replication
  out_final1 <- list.rbind(out_rep1)
  out_final2 <- list.rbind(out_rep2)
  out_final3 <- list.rbind(out_rep3)
  out_final <- list()
  out_final$out1 <- out_final1
  out_final$out2 <- out_final2
