...
for(rr in 1:rep_iter){
  ...
  for(q in 1:length(net_vec)){
    ...
    # Define the range for treatment and neighbors' treatments
    '%<-%' <- zeallot::'%<-%' 
    ...
    temp_train <- as.data.frame(cbind(unlist(nei_treat), unlist(Treat), list.rbind(est_median), dummy_grp))
    colnames(temp_train) <- c("nei_treat", "Treat", "est_Z1", "est_Z2", paste0("grp",1:num_grp))
    ...
    for(s in 1:sim_num){
      ...
      ################### LSM-BART
      df_train <- cbind(unlist(nei_treat), unlist(Treat), list.rbind(est_Z), dummy_grp)
      colnames(df_train) <- c("nei_treat","Treat", "est_Z1", "est_Z2", paste0("grp",1:num_grp))
      
      temp_mat <- cbind(unlist(Treat), list.rbind(est_Z), dummy_grp)
      rownames(temp_mat) <- NULL
      df_test <- data.frame(rep(nei_vec, each = N), temp_mat)
      colnames(df_test) <- c("nei_treat", "Treat", "est_Z1", "est_Z2", paste0("grp",1:num_grp))
    ...
    saveRDS(out, paste0("HLSM_BART", net_vec[q], "iter", rr, ".rds"))
  }
}
