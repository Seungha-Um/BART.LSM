...
for(rr in 1:rep_iter){
  ...
  for(tt in 1:2){
    ...
    for(q in 1:length(in_vec)){
        ...
        # out$df_net <- df_net
        # 
        # df1 <- ave_mu_test_list %>% filter(iter > num_burn) %>% 
        #         group_by(iter) %>%
        #         mutate(foo = lead(mean), IE = foo - mean) 
        # 
        # df2 <- data.frame(nei_treat = out$nei_vec, true_po = out$true_potential) %>%
        #   mutate(foo2 = lead(true_po), IE_true = foo2 - true_po)
        # 
        # df_com <- left_join(df1, df2, by = "nei_treat") 
        # 
        # df_temp <- df_com %>% filter(!is.na(IE)) %>% group_by(iter, nei_treat) %>% 
        #   summarize(RMSE_temp = (IE - IE_true)^2 ) %>%
        #   group_by(iter) %>% summarize(RMSE= sqrt(mean(RMSE_temp)))
        # 
        ...
        out$out1 <- out1
        out$out3 <- out3
        saveRDS(out, paste0("BART_incorrect","coef", in_vec[q], "treat", tt, "iter",rr, ".rds"))
      }
    }
  }
